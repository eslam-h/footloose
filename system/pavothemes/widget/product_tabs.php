...
class PtsWidgetProduct_tabs extends PtsWidgetPageBuilder {
		...
		private function getProducts( $results, $setting ){
            ...
            if ($this->config->get('out_of_stock_label_enabled')) {
                ...
                $system = $this->model_module_out_of_stock_label->getLabel((int) $this->config->get('config_language_id'));
            }
                ...
                foreach ($this->model_catalog_product->getProductCartOptions($result['product_id']) as $option) {
                    foreach ($option['product_option_value'] as $option_value) {
                        if ((!$option_value['subtract'] || ($option_value['quantity'] > 0))) {
                            $option_image = $this->model_tool_image->resize($option_value['image'], 47, 25);
...
//					}
                    ...
                    $product_option_value_data = array();
                    foreach ($this->model_catalog_product->getProductCartOptions($id_product) as $option) {
                        foreach ($option['product_option_value'] as $option_value) {
                            if ((!$option_value['subtract'] || ($option_value['quantity'] > 0))) {
                                $option_image = $this->model_tool_image->resize($option_value['image'], 47, 25);
                                if(!isset($option_image))
...
//						'thumb'   	 => $image,
		...
		public function renderContent( $args, $setting ){ 
			...
			if($setting['enable_newest']){ 
				$tabs['latest'] = $this->getProducts( $this->model_catalog_product->getLatestProducts( $data['limit'] ), $setting );
		 	}
			...
			$setting['tabs'] = $tabs;
			//echo "<pre>".print_r($setting['tabs'],1);die;

			$languageID = $this->config->get('config_language_id');
			
			$setting['objlang'] = $this->language;

            $setting['product_collection_style'] = $this->language->get('product_collection_style');
			
			$setting['heading_title'] = isset($setting['widget_title_'.$languageID])?$setting['widget_title_'.$languageID]:'';

            $setting['plus_pic'] = $this->model_tool_image->resize('catalog/plus-sign.png', 47, 25);
            $setting['text_more_details'] = $this->language->get('text_more_details');
