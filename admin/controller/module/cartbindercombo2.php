<?php
class Controllermodulecartbindercombo2 extends Controller {
	private $error = array();
 
	public function index() {
		$this->load->language('module/cartbindercombo2');
	...
	public function insert() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$this->response->redirect($this->url->link('module/cartbindercombo2', 'token=' . $this->session->data['token'].$url, 'SSL'));
		}
	...
	private function getList() {
		...
		if (isset($this->request->get['filter_status'])) {
  		...
  		$data['breadcrumbs'][] = array(
       		'href'      => $this->url->link('module/cartbindercombo2', 'token=' . $this->session->data['token'], 'SSL'),
       		'text'      => $this->language->get('heading_title'),
      		'separator' => ' :: '
   		);
							
		
		$data['insert']  = $this->url->link('module/cartbindercombo2/insert', 'token=' . $this->session->data['token'], 'SSL');
		$data['delete']  = $this->url->link('module/cartbindercombo2/delete', 'token=' . $this->session->data['token'], 'SSL');
		...
		foreach ($results as $result) {
			...
			$action[] = array(
				...
				'href' => $this->url->link('module/cartbindercombo2/update', 'token=' . $this->session->data['token'] . '&id=' . $result['id'], 'SSL')
			);		
			$data['cartbindercombo2s'][] = array(
				'id' 		 	 => $result['id'],
				'name' 	 	     => $result['name'],
		...
		$data['sort_name'] = $this->url->link('module/cartbindercombo2', 'token=' . $this->session->data['token'] . '&sort=name' . $url, 'SSL');
		$data['sort_status'] = $this->url->link('module/cartbindercombo2', 'token=' . $this->session->data['token'] . '&sort=status' . $url, 'SSL');
		...
		$pagination->url   = $this->url->link('module/cartbindercombo2/', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
		...
		$data['sort_name'] = $this->url->link('module/cartbindercombo2/', 'token=' . $this->session->data['token']. '&sort=c.name' . $url, 'SSL');
		$data['sort_status'] = $this->url->link('module/cartbindercombo2/', 'token=' . $this->session->data['token']. '&sort=c.status' . $url, 'SSL');

		$data['header'] = $this->load->controller('common/header');
		$data['column_left'] = $this->load->controller('common/column_left');
		$data['footer'] = $this->load->controller('common/footer');

		$this->response->setOutput($this->load->view('module/cartbindercombo2_list.tpl', $data));
 	}
	...
	private function getForm() {
		...
		if (!isset($this->request->get['id'])) {
			$data['action'] = $this->url->link('module/cartbindercombo2/insert', 'token=' . $this->session->data['token'], 'SSL');
		} else {
			$data['action'] = $this->url->link('module/cartbindercombo2/update', 'token=' . $this->session->data['token'] . '&id=' . $this->request->get['id'], 'SSL');
		}
		
		$data['token'] = $this->session->data['token'];
		  
    	$data['cancel'] = $this->url->link('module/cartbindercombo2', 'token=' . $this->session->data['token'], 'SSL');
    	$data['addoffers']  = $this->url->link('catalog/salescombopge', 'token=' . $this->session->data['token'], 'SSL');
		...
		$this->response->setOutput($this->load->view('module/cartbindercombo2_form.tpl', $data));
	}
	...
	private function validateForm() {
		...
		if (!$this->user->hasPermission('modify', 'module/cartbindercombo2')) {
