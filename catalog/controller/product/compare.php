    ...
    public function index() {
        ...
        foreach ($this->session->data['compare'] as $key => $product_id) {
            ...
            if ($product_info) {
                ...
                $attribute_groups = $this->model_catalog_productall->getProductAttributes($product_id);
                ...
                foreach ($this->model_catalog_productall->getProductCartOptions($product_info['product_id']) as $option) {
                    foreach ($option['product_option_value'] as $option_value) {
                        if ((!$option_value['subtract'] || $option_value['quantity'] > 0) ) {

                            $option_image = $this->model_tool_image->resize($option_value['image'], 75, 50);
...
//                $first_option = $this->model_catalog_productall->getFirstColorOptionSku($product_info['product_id']);
//
//                $product_cart_images = $this->model_catalog_productall->resizeProductCartImages($product_info['product_id'], $first_option, $this->config->get($this->config->get('config_theme') . '_image_product_width'), $this->config->get($this->config->get('config_theme') . '_image_product_height'));

                $optionsSku = $this->model_catalog_productall->getOptionsSku($product_info['product_id']);

                $product_cart_images = $this->model_catalog_productall->resizeProductCartImages($product_info['product_id'], $optionsSku, $this->config->get($this->config->get('config_theme') . '_image_product_width'), $this->config->get($this->config->get('config_theme') . '_image_product_height'));
