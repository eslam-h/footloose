...
class ModelTotalVoucher extends Model {
	...
	public function getVoucher($code) {
		$voucher_query = $this->db->query("SELECT *, vtd.name AS theme FROM " . DB_PREFIX . "voucher v LEFT JOIN " . DB_PREFIX . "voucher_theme vt ON (v.voucher_theme_id = vt.voucher_theme_id) LEFT JOIN " . DB_PREFIX . "voucher_theme_description vtd ON (vt.voucher_theme_id = vtd.voucher_theme_id) WHERE date_end > NOW() AND v.code = '" . $this->db->escape($code) . "' AND vtd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND v.status = '1'");

		if ($voucher_query->num_rows) {

            $special_status = false;
            foreach($this->cart->getProducts() as $product) {
                $product_special_query = $this->db->query("SELECT count(product_id) AS total FROM `" . DB_PREFIX . "product_special` ps WHERE ps.product_id = '" . (int)$product['product_id'] . "'");
                if($product_special_query->row['total'] == 0) {
                    $special_status = true;
                }
            }
	...
	public function getTotal($total) {
		if (isset($this->session->data['voucher'])) {
			...
			if ($voucher_info) {
			    $total_excluded = $total['total'];

                foreach($this->cart->getProducts() as $product) {
                    $product_special_query = $this->db->query("SELECT count(product_id) AS total FROM `" . DB_PREFIX . "product_special` ps WHERE ps.product_id = '" . (int)$product['product_id'] . "'");
                    if($product_special_query->row['total'] > 0) {
                        $total_excluded -= $product['total'];
                    }
                }
                ...
                if (isset($this->session->data['shipping_method'])) {
                    ...
                    $total_excluded -= $this->session->data['shipping_method']['cost'];
                }

				$amount = min($voucher_info['amount'], $total_excluded);
	...
	public function confirm($order_info, $order_total) {
		...
		if ($code) {
			...
			if ($voucher_info) {
                ...
                $this->db->query("UPDATE " . DB_PREFIX . "voucher SET status = '0' WHERE code = '" . $this->db->escape($code) . "'");
			} else {
