...
class ControllerCustomerCustomer extends Controller {
	...
	protected function getList() {
		...
		if (isset($this->request->get['filter_birth'])) {
			$filter_birth = $this->request->get['filter_birth'];
		} else {
			$filter_birth = null;
		}
...
$data['export'] = $this->url->link('customer/customer/export', 'user_token=' . $this->session->data['user_token'] . $url, true);
 ...
 public function export($getarr='') {
        ...
        $customers_column = array('Customer ID', 'Customer Name', 'Email', 'Telephone', 'Address', 'City', 'Post Code', 'Country', 'IP', 'Status', 'Date Added', 'Customer Group');
